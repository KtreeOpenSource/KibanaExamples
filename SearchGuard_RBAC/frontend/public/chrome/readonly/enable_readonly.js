...
import restrictedUrlConfig from "../../../lib/session/restrictedUrlConfig"
...
function resolveWithDashboardRole($q, $rootScope, $location, route, dashboardConfig, authInfo) {
    ...
    let numberOfTenants = Object.keys(authInfo.sg_tenants).length;
    ...
    if (numberOfTenants > 1) {
        allowedPaths.push('/app/searchguard-multitenancy')
    }
...
function resolveRegular(authInfo) {
    const isAdmin = authInfo.backend_roles.indexOf(CONSTANTS.admin) > -1
    const visibleMenuItems = ['kibana:dashboard']
    ...
    chrome.getNavLinks().forEach((navLink) => {
        ...
        if (!isAdmin) {
            if (visibleMenuItems.indexOf(navLink.id) == -1) {
                navLink.hidden = true
            }
        }
    });
...
function readOnlyResolver($q, $rootScope, $http, $location, route, dashboardConfig) {
    ...
    let dashBoardAccessConfig = chrome.getInjected('dashBoardAccess');
    let publicDashboardId = chrome.getInjected('publicDashboardId');
    ...
    return $http.get(`${API_ROOT}/auth/authinfo`)
        .then((response) => {
            ...
            window.sgAuthUserInfo = response.data
            window.kibanaDashBoardAccessConfig = dashBoardAccessConfig
            window.publicDashboardId = publicDashboardId

            // for hiding submenus for all users except admin
            var body = document.querySelector('body');
            if(authInfo.backend_roles.includes('admin') === false){
                body.classList.add('hideSideNavbar')
                // var clas  = document.querySelectorAll('body.hideSideNavbar, div.global-nav').display = 'none'
                // console.log('clas',clas)
            }

            let userReadOnlyRoles = authInfo.sg_roles.filter((role) => {
            ...
            if (authInfo.user_requested_tenant == GLOBAL_TENANT_VALUE) {
                ...
                if (
                    (authInfo.user_requested_tenant == PRIVATE_TENANT_VALUE && authInfo.sg_tenants[authInfo.user_name] === false)
                    ||
...
uiModules.get('searchguard').run(enableReadOnly);
