  ...
  componentDidMount() {
    ...
    getPageOfItems = () => {
      ...
      if(roleBasedDashboards.length !== 0) {
        dashboardsCopy = roleBasedDashboards.slice();
      }else if(loggeduserRoles.includes('admin') === true && roleBasedDashboards.length === 0) {
        dashboardsCopy = this.state.dashboards.slice();
      }else{
        dashboardsCopy = '';
      }
    ...
    logout(APP_ROOT) {
      ...
      window.location = APP_ROOT + '/../../login?type=basicauthLogout';
    }
    renderListing() {
      let createButton;
      let logoutButton;
      ...
      return (
        <div>
          ...
          {this.props.hideWriteControls ? '' : this.renderSearchBar()}
